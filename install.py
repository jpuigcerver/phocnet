...
def main(cudnn_dir, no_caffe, opencv_dir, install_dir, install_caffe_dir):
    ...
    if not no_caffe:
        ...
        os.makedirs('build')
        os.chdir('build')
        call_list = ['cmake', '..', '-DCMAKE_BUILD_TYPE=RELEASE', '-DCMAKE_INSTALL_PREFIX=%s' % install_caffe_dir]
        ...
        #call_list.append('-DPROTOBUF_PROTOC_EXECUTABLE=/home/jpuigcerver/.local/bin/protoc')
        #call_list.append('-DPROTOBUF_INCLUDE_DIR=/home/jpuigcerver/.local/include')
        #call_list.append('-DPROTOBUF_LIBRARY=/home/jpuigcerver/.local/lib/libprotobuf.so')
        #call_list.append('-DPROTOBUF_LIBRARY_DEBUG=/home/jpuigcerver/.local/lib/libprotobuf.so')
        #call_list.append('-DPROTOBUF_LITE_LIBRARY=/home/jpuigcerver/.local/lib/libprotobuf-lite.so')
        #call_list.append('-DPROTOBUF_LITE_LIBRARY_DEBUG=/home/jpuigcerver/.local/lib/libprotobuf-lite.so')
        #call_list.append('-DPROTOBUF_PROTOC_LIBRARY=/home/jpuigcerver/.local/lib/libprotoc.so')
        #call_list.append('-DPROTOBUF_PROTOC_LIBRARY_DEBUG=/home/jpuigcerver/.local/lib/libprotoc.so')
        if call(call_list) != 0:
            raise ValueError('Error during CMake run')
        # make
        logger.info('Compiling Caffe submodule...')
        if call(['make', '-j8', 'install']) != 0:
